...
EPrints::Plugin::Screen::Public::RequestArkivum

=cut

package EPrints::Plugin::Screen::Public::RequestArkivum;
...
sub render
{
    ...
    $page->appendChild( my $intro = $xml->create_element( "p" ) );
    $intro->appendChild( $self->html_phrase( "request_arkivum:intro" ) );

    my $mailto = "mailto:".$session->config( "adminemail" )."?subject=".$self->html_phrase( "request_arkivum:subject", eprint => $eprint->render_value( "title" ), document => $doc->render_value( "main" ) );
    $intro->appendChild( my $a = $xml->create_element( "a", href => $mailto ) );
    $a->appendChild( $session->make_text( $session->config( "adminemail" ) ) );
